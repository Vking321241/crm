...
//   on the list falls through to the talk.divary.com.br fallback with a
...
//
// Previous implementation hard-defaulted to `https://talk.divary.com.br`
...
function getBaseUrl(request: Request): string {
  ...
  return "https://talk.divary.com.br";
}
  ...
  try {
    ...
    const rows = await ctx.db
      .select({
        ...
        createdByUserId: authTokens.createdByUserId,
...
export async function POST(request: Request) {
  try {
    ...
    // Seat quota: only the platform owner can raise
    // `accounts.max_agent_seats` (via /admin), so this is the
    // enforcement point for "the admin can only create as many
    // users as the platform allows". Counts active members +
    // outstanding (un-redeemed, non-expired) invitations, since a
    // pending invite already reserves a seat.
